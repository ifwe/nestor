...
Jenkins.prototype._jobUrl = function(jobName) {
    return this.url + '/job/' + jobName;
};

Jenkins.prototype._buildUrl = function(jobName, buildNumber) {
    if (!buildNumber) {
        buildNumber = 'lastBuild';
    }

    return this._jobUrl(jobName) + '/' + buildNumber.toString();
};
...
Jenkins.prototype.consoleStream = function(jobName, opts, cb) {
  ...
  const INTERVAL = 1000;
  var url = this._buildUrl(jobName, 'lastBuild') + '/logText/progressiveText',
...
/**
 * Retrieve status and date for a build of a project.
 *
 * @param {String} name: Jenkins job name
 * @param {String-like} number: Anything with a 'toString()' that jenkins will understand
 * @param {Function} cb: standard cb(err, result) callback
 */
...
Jenkins.prototype.last = function (name, cb) {
    return this.getBuild(name, 'lastBuild', cb);
};
